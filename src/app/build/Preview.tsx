...
export default function Preview({ wheelTextureURLs, deckTextureURLs }: Props) {
  ...
  const { seletedWheel, seletedBolt, seletedDeck, seletedTruck } =
    useCustomizerContorls();

  const wheelTextureURL =
    asImageSrc(seletedWheel?.texture) ?? DEFAULT_WHEEL_TEXTURE;
  const deckTextureURL =
    asImageSrc(seletedDeck?.texture) ?? DEFAULT_DECK_TEXTURE;

  const truckColor = seletedTruck?.color ?? DEFAULT_TRUCK_COLOR;
  const boltColor = seletedBolt?.color ?? DEFAULT_BOLT_COLOR;
